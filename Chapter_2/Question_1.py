...
class IntArray():
    ...
    def insert(self, index:int, val:int) -> None:
        ...
        # Create new memory with updated size
        new_resmem = ReservedMemory(self._size * self._bytes_per_element)
        ...
        if self._resmem:
            ...
            if index < self._size - 1:
                source_start = index * self._bytes_per_element
                dest_start = (index + 1) * self._bytes_per_element
                count = (self._size - 1 - index) * self._bytes_per_element
                new_resmem.copy(self._resmem, count=count, source_index=source_start, destination_index=dest_start)
        
        # Update to the new memory
        self._resmem = new_resmem
        
        # Store the new value at the insertion point
